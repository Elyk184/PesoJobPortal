        ...
        <div class="content">
            ...
            <div class="body-text">
                <strong>THIS CERTIFIES FURTHER THAT</strong> based on the clearances issued by the BARANGAY <span id="residence-address-body" class="capitalize-words{{ $residenceAddress ? ' typed-style' : '' }}">
                        @if($clearance->status === 'pending')
                        <span id="residence-address-inline" class="clearance-inline-editor clearance-inline-editor--barangay" contenteditable="true" role="textbox" aria-label="Residence address" data-placeholder="TYPE BARANGAY / ADDRESS HERE">{{ old('residence_address', $residenceAddress) }}</span>
                    @else
                        {!! $residenceAddress ? e($residenceAddress) : '<span class="placeholder-uppercase">TYPE BARANGAY / ADDRESS HERE</span>' !!}
                    @endif
