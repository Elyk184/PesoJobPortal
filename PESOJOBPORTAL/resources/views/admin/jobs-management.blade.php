            ...
            <tbody>
                @forelse($jobs as $job)
                    <tr data-job-id="{{ $job->id }}" 
                        ...
                        data-job-description="{{ addslashes($job->description) }}"
                        data-job-qualifications="{{ addslashes($job->qualifications) }}"
                        data-job-responsibilities="{{ addslashes($job->key_responsibilities) }}"
                        data-job-skills="{{ addslashes($job->preferred_skills) }}"
                        data-job-experience="{{ addslashes($job->experience) }}"
                        data-job-education="{{ addslashes($job->education) }}"
                        data-job-benefits="{{ addslashes($job->benefits) }}"
                        data-job-company="{{ $job->employer?->companyProfile?->company_name ?? $job->employer_name ?? 'N/A' }}"
                        data-job-location="{{ $job->location }}"
                        data-job-type="{{ $job->job_type }}"
                        data-job-vacancies="{{ $job->vacancies }}"
                        data-job-salary="{{ $job->salary_range ?? $job->salary }}"
                        data-job-requirements="{{ addslashes($job->requirements) }}"
                        data-job-start-date="{{ $job->application_start_date?->format('Y-m-d') }}"
                        data-job-end-date="{{ $job->application_end_date?->format('Y-m-d') }}"
                        data-job-applications="{{ $job->applications?->count() ?? 0 }}"
                        data-job-filled="{{ $job->is_filled ? '1' : '0' }}"
                        ...
                        <td class="job-location">{{ $job->location ?? 'N/A' }}</td>
                        <td><span class="job-type-badge"><i class="bi bi-clock-history"></i>{{ ucfirst(str_replace('_', ' ', $job->job_type ?? 'N/A')) }}</span></td>
                        <td class="job-vacancies">{{ $job->vacancies ?? 0 }}</td>
                        <td class="job-salary">{{ $job->salary_range ?? $job->salary ?? 'N/A' }}</td>
                        <td class="job-applications">{{ $job->applications?->count() ?? 0 }}</td>
                        <td class="job-deadline">{{ $job->application_end_date?->format('d M Y') ?? 'N/A' }}</td>
                        <td class="job-requirements" title="{{ $job->requirements ?? '' }}">{{ $job->requirements ?? 'N/A' }}</td>
