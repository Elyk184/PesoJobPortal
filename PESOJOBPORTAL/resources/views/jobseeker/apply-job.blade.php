...
@section('title', ($job->title ?? 'Job Details') . ' | PESO')
@section('page-title', 'Job Details')
@section('page-subtitle', 'View the full job post')
...
<section aria-label="Job details" class="job-detail-page">
    ...
    <a href="{{ route('jobseeker.browse-jobs') }}" class="back-link">
    ...
    <div class="job-detail-grid">
        <article class="job-detail-card">
            <div class="job-detail-header">
                <div>
                    ...
                    <div class="job-meta">
                        <span class="job-pill"><i class="bi bi-building"></i>{{ $job->employer_name ?? $job->employer?->name ?? 'Company' }}</span>
                        <span class="job-pill"><i class="bi bi-geo-alt"></i>{{ $job->location ?? 'Location not listed' }}</span>
                        <span class="job-pill"><i class="bi bi-clock"></i>{{ ucfirst(str_replace('_', ' ', $job->job_type ?? $job->employment_type ?? 'N/A')) }}</span>
                        @if($job->salary_range)
                            <span class="job-pill"><i class="bi bi-cash-stack"></i>{{ $job->salary_range }}</span>
                        @endif
                    </div>
                </div>
                <div class="text-md-end">
                    <div class="text-muted small">Posted {{ $job->created_at?->diffForHumans() }}</div>
                    @if($job->application_end_date)
                        <div class="mt-1 fw-bold {{ $job->application_end_date->isPast() ? 'text-danger' : 'text-success' }}">
                            {{ $job->application_end_date->isPast() ? 'Expired' : 'Expires ' . $job->application_end_date->format('M d, Y') }}
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="job-section">
                ...
                <div class="job-text">{{ $job->description ?? 'No description provided.' }}</div>
            </div>

            <div class="job-section">
                <h3>Qualifications</h3>
                <div class="job-text">{{ $job->qualifications ?? 'No qualifications listed.' }}</div>
            </div>

            @if(! empty($job->key_responsibilities))
                <div class="job-section">
                    <h3>Key Responsibilities</h3>
                    <div class="job-text">{{ $job->key_responsibilities }}</div>
                </div>
            @endif
            ...
            @if(! empty($job->preferred_skills))
                <div class="job-section">
                    ...
                    <div class="job-text">{{ $job->preferred_skills }}</div>
                </div>
            @endif
            ...
            <div class="job-summary-mini">
                ...
                <div class="mini-row">
                    ...
                    <span class="mini-value">{{ $job->employer_name ?? $job->employer?->name ?? 'Company' }}</span>
                </div>
                <div class="mini-row">
                    <span class="mini-label">Vacancies</span>
                    <span class="mini-value">{{ $job->vacancies ?? 1 }}</span>
                </div>
                <div class="mini-row">
                    <span class="mini-label">Employment Type</span>
                    <span class="mini-value">{{ ucfirst(str_replace('_', ' ', $job->job_type ?? $job->employment_type ?? 'N/A')) }}</span>
                </div>
                @if($job->salary_range)
                <div class="mini-row">
                    <span class="mini-label">Salary Range</span>
                    <span class="mini-value">{{ $job->salary_range }}</span>
                </div>
                @endif
            </div>
        </article>
        ...
        <aside>
            ...
            <div class="application-form-card">
                ...
                <form action="{{ route('jobseeker.submit-application', $job) }}" method="POST" enctype="multipart/form-data" id="applicationForm">
                    @csrf
                    ...
                    <input type="hidden" id="resumeType" name="resume_type" value="{{ $defaultResumeType }}">
                    <input type="hidden" id="resumeBuilderSelect" name="use_resume_builder" value="{{ $defaultResumeType === 'builder' ? '1' : '0' }}">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea
                            ...
                            maxlength="2000">{{ old('letter') }}</textarea>
                        <small style="display: block; margin-top: 0.4rem; opacity: 0.85; font-size: 0.8rem;">
                            <span id="char-count">0</span>/2000 characters
                        </small>
                        @error('letter')
                            <div class="text-danger small mt-2" style="background: rgba(220, 53, 69, 0.2); padding: 0.5rem; border-radius: 8px; color: #ffcdd2;">{{ $message }}</div>
                        @enderror
                    </div>
