...
@section('title', strtoupper($activityRequest->activity_type) . ' Request Details')
...
<style>
    ...
    .meta-item {
        ...
        padding: 12px;
        border-radius: 8px;
        border-left: 3px solid {{ $activityRequest->activity_type === 'lra' ? '#3b82f6' : '#ec4899' }};
    }
...
<div style="padding: 20px;">
    <a href="{{ route('employer.recruitment.index') }}" class="back-btn">
    ...
    <div class="detail-header">
        <div style="display: flex; justify-content: space-between; align-items: start;">
            <div>
                <h1>{{ strtoupper($activityRequest->activity_type) }} Request</h1>
                <span class="status-badge status-{{ $activityRequest->status }}">
        ...
        <div class="header-meta">
            <div class="meta-item">
                ...
                <div class="meta-value">#{{ $activityRequest->id }}</div>
            </div>
            <div class="meta-item">
                <div class="meta-label">Activity Type</div>
                <div class="meta-value">{{ strtoupper($activityRequest->activity_type) }}</div>
            </div>
            <div class="meta-item">
                <div class="meta-label">Submitted</div>
                <div class="meta-value">{{ $activityRequest->created_at->format('M d, Y') }}</div>
            </div>
            @if($activityRequest->status !== 'pending')
                <div class="meta-item">
                    <div class="meta-label">{{ $activityRequest->status === 'approved' ? 'Approved' : 'Rejected' }}</div>
                    <div class="meta-value">{{ optional($activityRequest->updated_at)->format('M d, Y') }}</div>
                </div>
            @endif
        </div>
    </div>
    ...
    @if($activityRequest->status === 'approved')
        <div class="certificate-section">
            ...
            <div class="certificate-info">
                ...
                <p><strong>Generated:</strong> {{ optional($activityRequest->certification_generated_at)->format('M d, Y H:i A') }}</p>
                @if($activityRequest->approvedBy)
                    <p><strong>Approved by:</strong> {{ $activityRequest->approvedBy->name }}</p>
                @endif
            </div>
            <div class="certificate-actions">
                <a href="{{ route('employer.recruitment.view-certificate', $activityRequest) }}" target="_blank" class="btn-primary">
                    <i class="bi bi-eye me-2"></i>View Certificate
                </a>
                <a href="{{ route('employer.recruitment.download-certificate', $activityRequest) }}" class="btn-primary" style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%);">
    ...
    @elseif($activityRequest->status === 'rejected')
        <div style="background: linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%); border: 1px solid #fecaca; border-radius: 12px; padding: 20px; margin-bottom: 24px; border-left: 4px solid #ef4444;">
            ...
            <div class="rejection-reason">
                ...
                <p style="font-style: italic; margin-top: 8px;">{{ $activityRequest->notes ?? 'No reason provided' }}</p>
            </div>
        </div>
    @else
        ...
        <div class="document-grid">
            @if($activityRequest->letter_of_intent_path)
                <div class="document-item">
                    ...
                    <a href="{{ asset('storage/' . $activityRequest->letter_of_intent_path) }}" class="download-btn" target="_blank">Download</a>
                </div>
            @endif
            ...
            @if($activityRequest->activity_type === 'lra')
                @if($activityRequest->business_permit_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->business_permit_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->lra_recruitment_officer_id_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->lra_recruitment_officer_id_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->job_vacancies_path)
                    <div class="document-item">
                        <div class="document-icon"><i class="bi bi-file-pdf"></i></div>
                        <div class="document-name">Job Vacancies</div>
                        <a href="{{ asset('storage/' . $activityRequest->job_vacancies_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
            @endif
            ...
            @if($activityRequest->activity_type === 'sra')
                @if($activityRequest->dmw_certificate_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->dmw_certificate_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->recruitment_officer_id_path)
                    <div class="document-item">
                        <div class="document-icon"><i class="bi bi-file-pdf"></i></div>
                        <div class="document-name">Officer ID</div>
                        <a href="{{ asset('storage/' . $activityRequest->recruitment_officer_id_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->job_order_balance_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->job_order_balance_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->deployment_report_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->deployment_report_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->affidavit_undertaking_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->affidavit_undertaking_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
                @if($activityRequest->sra_authority_file_path)
                    <div class="document-item">
                        ...
                        <a href="{{ asset('storage/' . $activityRequest->sra_authority_file_path) }}" class="download-btn" target="_blank">Download</a>
                    </div>
                @endif
            @endif
        </div>
    ...
    @if($activityRequest->activity_type === 'lra' && $activityRequest->job_vacancies_text)
        <div class="document-card">
