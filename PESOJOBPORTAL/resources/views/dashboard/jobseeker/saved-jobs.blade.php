        ...
        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
            <div>
                <h2 class="h4 mb-1 fw-bold">
                    Saved Jobs
                    <span class="badge bg-light text-dark border ms-2" id="savedCountBadge">{{ $savedCount }}</span>
                </h2>
                ...
                @foreach ($savedJobs as $job)
                    <div class="col-12">
                        <article class="saved-job-item p-3 p-lg-4">
                            <div class="saved-job-top">
                                ...
                                <div class="saved-job-main">
                                    ...
                                    <div class="saved-job-subtitle">
                                        <span><i class="bi bi-building me-1"></i>{{ $job['employer_name'] }}</span>
                                        <span class="mx-2 text-muted">|</span>
                                        <span><i class="bi bi-geo-alt me-1"></i>{{ $job['location'] }}</span>
                                    </div>
                                </div>
                                <div class="saved-job-actions">
                                    <a href="{{ route('jobseeker.apply-job', $job['id']) }}" class="btn btn-primary btn-sm px-3">
                                        View
                                    </a>
                                    <button type="button" class="btn btn-outline-danger btn-sm px-3" onclick="toggleSaveJob({{ $job['id'] }}, this)" title="Remove from saved jobs">
                            ...
                            <div class="saved-job-desc">{{ \Illuminate\Support\Str::limit($job['description'], 160) }}</div>

                            @if (! empty($job['requirements_list']))
                                <div class="saved-job-tags">
                                    @foreach (collect($job['requirements_list'])->take(2) as $requirement)
