...
@section('title', 'Notifications | Jobseeker')
	...
	<div class="gmail-shell">
		<div class="gmail-toolbar">
			...
			<div class="gmail-toolbar-counts">
				<span class="gmail-meta all"><i class="bi bi-inboxes"></i><span id="notificationsTotalMeta" data-count="{{ $notifications->count() }}">{{ $notifications->count() }} total</span></span>
				<span class="gmail-meta"><i class="bi bi-circle-fill"></i><span id="notificationsUnreadMeta">{{ $unreadCount }} unread</span></span>
		...
		<div id="notificationsList" class="gmail-list" data-latest-id="{{ $latestNotificationId }}">
			@forelse ($notifications as $notification)
				...
				@endphp
				<div class="gmail-row {{ $isUnread ? 'unread' : 'read' }}" data-notification-id="{{ $notification->id }}">
					@if ($isUnread)
					...
					<div class="gmail-subject">{{ $title }}</div>
					<div class="gmail-message">{{ $message }}</div>
					<span class="gmail-time">{{ optional($createdAt)->diffForHumans() ?? 'Now' }}</span>
					<span class="gmail-badge">{{ $isAdminRecommendation ? 'RECOMMEND' : 'PESO' }}</span>
					...
					@if ($isUnread)
						@if ($linkedJobId)
							<a href="{{ route('jobseeker.apply-job', $linkedJobId) }}" class="mark-read-btn gmail-action">View Job</a>
						@else
							<button type="button" class="mark-read-btn gmail-action" data-mark-read data-id="{{ $notification->id }}">Mark Read</button>
						@endif
					@else
